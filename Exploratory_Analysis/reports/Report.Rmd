...
In total, `r (enrolcount)` unique student enrolment records were assessed from seven survey files. The first survey file had the most enrolment records. The 'enrolled at' dates within the file indicate the first intake ran for the longest time.
...
The 'enrolled at' dates within the files  indicate that the courses overlapped in duration and varied in length, with the volumes of students applying for each course gradually reducing. Whilst enrolments within file four started after the third intake the last enrolment in the fourth file was prior to the last enrolment in file three. 
...
No. of students that shared data: `r format(Shared_Table_rows)`
For students who shared data, they shared an average of `r format(mean(enrol$Shared_Info))` items.
No. of students sharing gender: `r sum(Shared_Table$Shared_Gender)`
No. of students sharing age range: `r sum(Shared_Table$Shared_Age_Range)`
No. of students sharing employment status: `r sum(Shared_Table$Shared_Emp_Status)`
No. of students sharing employment area: `r sum(Shared_Table$Shared_Emp_Area)`
No. of students sharing highest education level: `r sum(Shared_Table$Shared_Highest_Ed)`
